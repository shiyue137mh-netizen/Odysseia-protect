...
5.  **【可选动态溯源】**: 作者可以为 PNG 角色卡主动开启动态溯源。开启后，Bot 只在下载者明确确认时生成个性化副本，并在副本中写入经过加密、且绑定具体作品的溯源凭证；凭证不包含明文 Discord 用户 ID。

**Bot 如何使用这些信息？**
-   这些信息仅用于实现 Bot 的核心功能，即文件的上传、下载和管理。
-   您的数据**不会**与任何第三方分享。
-   存储在私密仓库频道的文件副本仅用于生成下载链接，不会被用于任何其他目的。
-   动态溯源仅用于发现作品已在外部泄露后的被动样本核验。Bot 不读取成员本地文件，也不建立每次下载者身份的长期映射账本。
...
*   **方法一：使用命令**
    *   命令: `/上传`
    *   步骤: 选择“受保护文件”，**上传文件附件**，填写版本信息、可选密码，以及是否开启动态溯源。
*   **方法二：使用右键菜单 (推荐)**
...
*   **流程**：
    ...
    3.  验证通过后，Bot 会发送**有时效性的私密下载链接**；开启溯源的角色卡在首轮试验中会作为私密附件发送。

#### **4. 动态溯源（作者可选）**
*   首期仅支持带有 `chara` 或 `ccv3` 数据的 PNG 角色卡。
*   下载者会先看到完整告知；只有明确同意后，Bot 才会生成带加密凭证的个性化副本。
*   溯源是泄露后的调查线索，不是实时监控，也不代表系统能够阻止所有主动清洗行为。
